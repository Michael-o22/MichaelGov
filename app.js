...
const CONFIG = {
    ...
    // กำหนดขนาดไฟล์สูงสุดที่อนุญาต (25MB)
...
let docDatePicker = null; // ปฏิทินวันที่ในหนังสือ (พ.ศ.)
let filterStartDatePicker = null; // ปฏิทินตัวกรองวันที่เริ่มต้น
let filterEndDatePicker = null; // ปฏิทินตัวกรองวันที่สิ้นสุด
...
/**
 * ฟังก์ชันสร้าง Flatpickr พร้อมแปลงปี ค.ศ. เป็น พ.ศ. (Buddhist Era)
 */
function initThaiDatePicker(elementOrSelector, options = {}) {
    if (typeof flatpickr === 'undefined') return null;

    const defaultOptions = {
        locale: 'th',
        dateFormat: 'Y-m-d', // เก็บค่าจริงเป็น ISO YYYY-MM-DD
        altInput: true,
        altFormat: 'd/m/Y', // แสดงผลเป็น วว/ดด/ปปปป (พ.ศ.)
        altInputClass: 'w-full border-slate-300 rounded-lg shadow-sm focus:ring-emerald-500 focus:border-emerald-500 py-2 px-3 border text-sm bg-white cursor-pointer font-sarabun',
        allowInput: false,
        disableMobile: "true",
        ...
        parseDate: function(dateStr, format) {
            ...
            if (typeof dateStr === 'string') {
                ...
                if (/^\d{1,2}\/\d{1,2}\/\d{4}$/.test(dateStr)) {
                    ...
                    return new Date(yCE, m - 1, d);
                }
            }
            return new Date(dateStr);
        },
        onReady: function(selectedDates, dateStr, instance) {
            updateYearHeader(instance);
        },
    ...
    function updateYearHeader(instance) {
        ...
        if (!instance._isBeHooked) {
            instance._isBeHooked = true;
            ...
            instance.changeYear = function(year, isRelative) {
                origChangeYear(year, isRelative);
                setYearBE();
            };
        }
    }
    ...
    return flatpickr(elementOrSelector, mergedOptions);
}
...
function validateFile(file) {
    ...
    if (file.size > CONFIG.MAX_FILE_SIZE_BYTES) {
        ...
        const maxMB = (CONFIG.MAX_FILE_SIZE_BYTES / (1024 * 1024)).toFixed(0);
        return {
            valid: false,
            message: `ไฟล์มีขนาด ${sizeMB}MB ซึ่งเกินกว่าที่ระบบกำหนด (สูงสุด ${maxMB}MB)`
        };
    }
    return { valid: true };
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // กำหนดค่าตัวเลือกวันที่ภาษาไทย (ปี พ.ศ.)
    docDatePicker = initThaiDatePicker('#docDate');
    filterStartDatePicker = initThaiDatePicker('#filter-date-start');
    filterEndDatePicker = initThaiDatePicker('#filter-date-end');
    ...
    function openModal(isEdit = false) {
        if (!isEdit) {
            ...
            if (docDatePicker) docDatePicker.setDate(new Date(), true);
    ...
    function closeModal() {
        ...
        setTimeout(() => {
            ...
            editDocId.value = '';
            fileInput.value = '';
            if (docDatePicker) docDatePicker.clear();
            fileNameDisplay.textContent = '';
            fileNameDisplay.classList.add('hidden');
        }, 300);
    }
    ...
    window.editDocument = function(id) {
        ...
        if (docDatePicker) {
            docDatePicker.setDate(doc.date || '', true);
        } else {
